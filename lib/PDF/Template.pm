...
This will create a new instance of PDF::Template. $opts is an optional hashref
...
This will set the parameters that PDF::Template will use to merge the template
...
there should be a new page is something PDF::Template works very hard at
...
Currently, the only PDF renderer PDF::Template supports is PDFlib (available at
...
Provided the impetus to move to L<PDF::Writer> (which he also wrote).
